...
class EditBookController: NSViewController
{
    ...
    @IBAction func toggleSidebar(_ sender: NSToolbarItem) {
        ...
        animateToggleSidebar(duration: 1, newWidth: newWidth, item: sender)
    }
    
    func animateToggleSidebar(duration: TimeInterval, newWidth: CGFloat, item: NSToolbarItem) {
        guard let window = view.window else { return }
        ...
        NSAnimationContext.runAnimationGroup({ _ in
            ...
            item.label = self.isSplitViewCollapsed ? ToolbarLabels.showSidebar : ToolbarLabels.hideSidebar
        })
    }
    ...
    @IBAction func removeRow(_ sender: Any) {
        ...
        dataSource.removeBook(at: index)
